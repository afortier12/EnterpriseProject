...
public class DataLoad {
	...
	private static void loadFromCSV(File dir) {
	...
	private static void combineDataSets(File dir) {
		...
		try {
			...
			for (int i=1; i<arffFiles.size(); i++) {
			 ...
			 // save ARFF
		    ArffSaver saver = new ArffSaver();
		    saver.setInstances(completeData);
		    saver.setFile(new File(dir+"/complete.arff"));
		    saver.writeBatch();	
		} catch (IOException e) {
			System.out.println(e.getMessage());
		}
